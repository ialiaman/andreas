...
// FUNCTION GET ALL LEADS
const getAllLeadsForClient = (owner_id) => {
  const query = `SELECT * FROM LEADS WHERE owner_id='${owner_id}'`;
...
app.post("/signup", (req, res) => {
  ...
  const { fname, lname, email, password, company, companyUrl } = req.body;
  ...
  con.query(search, function (err, result) {
    if (err) {
      ...
      if (result.length > 0) {
        ...
        const sql = `INSERT INTO registered_users (f_name, l_name,email,password,c_name, company_url) VALUES ('${fname}', '${lname}', '${email}', '${md5Pasword}','${company}', '${companyUrl}')`;
        ...
        con.query(sql, function (err, result) {
          if (err) {
            ...
            const getOwnerId = `SELECT * FROM registered_users where email = '${email}'`;
            con.query(getOwnerId, function (err, result) {
              const insertDomain = `INSERT INTO domains (owner_id, domain) VALUES('${result[0].id}', '${companyUrl}')`;
              con.query(insertDomain, function (err, result) {
                if (err) {
                  throw err;
                } else {
