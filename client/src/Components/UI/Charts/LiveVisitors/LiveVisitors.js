...
function PageViews(props) {
  ...
  useEffect(() => {
    ...
    axios.post('http://localhost:3001/chats/leadschart', { c_name: authState.LoggedUserData.c_name }).then(res => {
      const data = res.data
      console.log(data)
      const leads = []
      for (let i = 0; i < data.length; i++) {
        ...
        leads.push( {x:data[i].Date.slice(0,10),y:data[i].no_of_rows} )
